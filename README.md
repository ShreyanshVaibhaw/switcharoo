...
switcharoo ships four pinned agents plus a routing policy skill that teaches the main loop when to use each:
...
```
/plugin  →  add this repo as a marketplace  →  install switcharoo
```
...
- The main loop's model is **not** auto-switched mid-conversation. That is architecturally reserved to your `/model` choice, by design and by ToS. What switcharoo automates is pushing subtasks down-tier - which is where most tokens go anyway.
